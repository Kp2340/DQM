...
function FormatCheck() {
    ...
    const handleFileUpload = async (event) => {
        ...
        try {
            const response = await axios.post('http://dqm-iw1g.onrender.com/upload/f', formData);
    ...
    const handleCalculate = async () => {
        ...
        try {
            const response = await axios.post(`http://dqm-iw1g.onrender.com/calculate/${selectedFormat}`, formData, {
                headers: {
    ...
    const handleSave = async () => {
        try {
            await axios.post(`http://dqm-iw1g.onrender.com/save/${selectedFormat}`);
            axios.get(`http://dqm-iw1g.onrender.com/all/${selectedFormat}`).then(response => {
    ...
    const handleDelete = async (id) => {
        try {
            console.log(id);
            await axios.delete(`http://dqm-iw1g.onrender.com/delete/f/${id}`);
    ...
    const handleDeleteAll = async (str) => {
        try {
            await axios.delete(`http://dqm-iw1g.onrender.com/delete/f/all/${str}`);
            axios.get(`http://dqm-iw1g.onrender.com/all/${selectedFormat}`).then(response => {
                if (response && response.data) {
                    setLogs(response.data);
                }
    ...
    return(
        ...
        <header style={{backgroundColor: '#4B2E83', padding: '15px 20px', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 2px 5px rgba(0, 0, 0, 0.1)', borderRadius: '5px', marginBottom: '20px',}}>
            ...
            <a href={'http://dqm-iw1g.onrender.com/domain'} style={{marginRight: '25px', color: "white", textDecoration: "none"}}>Domain Check</a>
            <a href={'http://dqm-iw1g.onrender.com/completeness'} style={{color: "white", textDecoration: "none"}}>Completeness Check</a>
        </header>
            <div style={{ textAlign: "center", marginTop: "20px" }}>
                <select
                    ...
                    onChange={(e) => {
                        ...
                        axios.get(`http://dqm-iw1g.onrender.com/all/${e.target.value}`)
                            .then(response => {
                                if (response && response.data) {
                                    setLogs(response.data);
                                }
